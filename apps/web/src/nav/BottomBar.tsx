...
import { ALL_DESTINATIONS, activeSectionFor } from './navItems.js';
  ...
  return (
    ...
    >
      {ALL_DESTINATIONS.map((item) => {
        const active = section === item.id || (item.id === 'settings' && section === 'admin');
        return (
          <NavLink
            key={item.id}
            to={item.path}
            aria-current={active ? 'page' : undefined}
            className={`flex flex-1 flex-col items-center gap-0.5 py-2 text-[10.5px] font-semibold no-underline focus:outline-none ${active ? 'text-[var(--color-text)]' : 'text-[var(--color-muted)]'}`}
          >
